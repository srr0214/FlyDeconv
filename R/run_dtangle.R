run_dtangle <- function(
  ...
  total_mem <- peakRAM::peakRAM({
    ...
    ## -------- 3. 后处理（非 core）--------
    prop_dtangle <- t(decon$proportions$dtangle_sig1)
    
    ## 去负值
    prop_dtangle[prop_dtangle < 0] <- 0
    
    ## 列归一化
    prop_dtangle <- sweep(
      prop_dtangle, 2, colSums(prop_dtangle), FUN = "/"
    )
    
    prop_dtangle <- as.data.frame(prop_dtangle)
  })
  ...
  utils::write.csv(prop_dtangle, prop_file)
