run_DESeq2_L1 <- function(
  ...
  if (!requireNamespace("DESeq2", quietly = TRUE)) {
    stop("Method DESeq2_L1 requires package 'DESeq2'.")
  }
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    stop("Method DESeq2_L1 requires package 'peakRAM'.")
  }
  ...
  total_mem <- peakRAM::peakRAM({
    ...
    ## ---------- Step 3: 后处理 ----------
    prop_DESeq2_L1 <- t(mix)
    prop_DESeq2_L1[prop_DESeq2_L1 < 0] <- 0
    prop_DESeq2_L1 <- sweep(
      prop_DESeq2_L1,
      2,
      colSums(prop_DESeq2_L1),
      "/"
    )
    prop_DESeq2_L1 <- as.data.frame(prop_DESeq2_L1)
  })
  ...
  utils::write.csv(prop_DESeq2_L1, prop_file)
