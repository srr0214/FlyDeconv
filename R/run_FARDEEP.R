run_FARDEEP <- function(
  ...
  if (!requireNamespace("FARDEEP", quietly = TRUE)) {
    stop("Method FARDEEP requires package 'FARDEEP'.")
  }
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    stop("Method FARDEEP requires package 'peakRAM'.")
  }
  ...
  total_mem <- peakRAM::peakRAM({
    ...
    X <- ref_expr[common_genes, , drop = FALSE]
    Y <- bulk[common_genes, , drop = FALSE]
    
    ## ---------- Step 2: 核心 FARDEEP ----------
    core_time <- system.time({
      core_mem <- peakRAM::peakRAM({
        res <- FARDEEP::fardeep(X, Y)
      })
    })
    
    ## ---------- Step 3: 后处理 ----------
    prop_FARDEEP <- t(res$relative.beta)
    prop_FARDEEP[prop_FARDEEP < 0] <- 0
    prop_FARDEEP <- sweep(
      prop_FARDEEP,
      2,
      colSums(prop_FARDEEP),
      "/"
    )
    prop_FARDEEP <- as.data.frame(prop_FARDEEP)
  })
  ...
  write.csv(prop_FARDEEP, prop_file)
