run_ProportionsInAdmixture <- function(
    bulk,
    tissue,
  ...
  ## =========================
  sex <- match.arg(sex)
  method_name <- "ProportionsInAdmixture"
  ...
  total_mem <- peakRAM::peakRAM({
    ...
    m <- bulk[common_genes, , drop = FALSE]        # bulk
    B <- celltype_expr[common_genes, , drop = FALSE]  # reference
    
    ## -------- 2. 核心步骤（ADAPTS）--------
    core_time <- system.time({
      core_mem <- peakRAM::peakRAM({
        
        cellEst <- ADAPTS::estCellPercent.proportionsInAdmixture(
          refExpr  = B,
          geneExpr = m
        )
    ...
    ## -------- 3. 后处理（非 core）--------
    prop_proportionsInAdmixture <- sweep(
      cellEst, 2, colSums(cellEst), FUN = "/"
    )
    
    prop_proportionsInAdmixture <- as.data.frame(prop_proportionsInAdmixture)
  })
  ...
  utils::write.csv(prop_proportionsInAdmixture, prop_file)
  ...
  invisible(list(
    prop    = prop_proportionsInAdmixture,
