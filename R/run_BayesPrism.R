run_BayesPrism <- function(
    bulk,
    tissue,
    sex = c("mix", "male", "female"),
    bulk_name = NULL,
    out_base = "results"
) {
  
  ## =========================
  ## 0. 参数处理
  ## =========================
  sex <- match.arg(sex)
  method_name <- "BayesPrism"
  
  if (is.null(bulk_name)) {
    bulk_name <- deparse(substitute(bulk))
  }
  
  ## =========================
  ## 1. 依赖检查（不 attach）
  ## =========================
  if (!requireNamespace("BayesPrism", quietly = TRUE)) {
    stop("Method BayesPrism requires package 'BayesPrism'.")
  }
  if (!requireNamespace("peakRAM", quietly = TRUE)) {
    stop("Method BayesPrism requires package 'peakRAM'.")
  }
  
  ## =========================
  ## 2. 输出路径 & benchmark
  ## =========================
  prop_dir <- file.path(out_base, method_name)
  log_file <- file.path(out_base, "runtime_benchmark.csv")
  dir.create(prop_dir, recursive = TRUE, showWarnings = FALSE)
  
  if (!file.exists(log_file)) {
    runtime_log <- data.frame(
      method = character(),
      bulk   = character(),
      core_time_sec  = numeric(),
      total_time_sec = numeric(),
      core_mem_MB    = numeric(),
      total_mem_MB   = numeric(),
      stringsAsFactors = FALSE
    )
  } else {
    runtime_log <- read.csv(log_file, stringsAsFactors = FALSE)
  }
  
  ## =========================
  ## 3. 获取 sc reference & annotation
  ## =========================
  sc_expr <- get_internal_data(
    tissue = tissue,
    sex    = sex,
    type   = "cell_expr"
  )
  
  sc_ann <- get_internal_data(
    tissue = tissue,
    sex    = sex,
    type   = "annotation"
  )
  
  ## =========================
  ## 4. 总体时间 & 内存
  ## =========================
  t_total_start <- Sys.time()
  
  total_mem <- peakRAM::peakRAM({
    
    ## ---------- Step 1: 基因交集 ----------
    common_genes <- intersect(
      rownames(bulk),
      rownames(sc_expr)
    )
    
    bulk_use <- bulk[common_genes, , drop = FALSE]
    sc_use   <- sc_expr[common_genes, , drop = FALSE]
    
    ## ---------- Step 2: 转置（BayesPrism 要求） ----------
    bulk_mat <- t(bulk_use)   # sample × gene
    sc_mat   <- t(sc_use)     # cell × gene
    
    ## ---------- Step 3: 构建标签 ----------
    cell.type.labels  <- sc_ann[colnames(sc_use), "cellType"]
    cell.state.labels <- cell.type.labels
    
    ## ---------- Step 4: 构建 prism（非核心） ----------
    myPrism <- BayesPrism::new.prism(
      reference = sc_mat,
      mixture   = bulk_mat,
      input.type = "count.matrix",
      cell.type.labels  = cell.type.labels,
      cell.state.labels = cell.state.labels,
      key = NULL,
      outlier.cut = 0.01,
      outlier.fraction = 0.1
    )
    
    ## ---------- Step 5: 核心步骤（Gibbs sampling） ----------
    core_time <- system.time({
      core_mem <- peakRAM::peakRAM({
        
        bp.res <- BayesPrism::run.prism(
          prism   = myPrism,
          n.cores = 4
        )
        
      })
    })
    
    ## ---------- Step 6: 提取比例 ----------
    theta <- BayesPrism::get.fraction(
      bp = bp.res,
      which.theta = "final",
      state.or.type = "type"
    )
    
    prop <- as.data.frame(t(theta))
  })
  
  t_total_end <- Sys.time()
  
  ## =========================
  ## 5. 时间 & 内存
  ## =========================
  core_time_sec  <- unname(core_time["elapsed"])
  total_time_sec <- as.numeric(
    difftime(t_total_end, t_total_start, units = "secs")
  )
  
  core_mem_MB  <- core_mem$Peak_RAM_Used_MiB[1]
  total_mem_MB <- total_mem$Peak_RAM_Used_MiB[1]
  
  ## =========================
  ## 6. 保存比例矩阵
  ## =========================
  prop_file <- file.path(
    prop_dir,
    paste0("prop_", method_name, "_", bulk_name, ".csv")
  )
  write.csv(prop, prop_file)
  
  ## =========================
  ## 7. 写 benchmark
  ## =========================
  runtime_log <- rbind(
    runtime_log,
    data.frame(
      method = method_name,
      bulk   = bulk_name,
      core_time_sec  = core_time_sec,
      total_time_sec = total_time_sec,
      core_mem_MB    = core_mem_MB,
      total_mem_MB   = total_mem_MB,
      stringsAsFactors = FALSE
    )
  )
  
  write.csv(runtime_log, log_file, row.names = FALSE)
  
  ## =========================
  ## 8. 返回
  ## =========================
  invisible(list(
    prop    = prop,
    runtime = runtime_log[nrow(runtime_log), ],
    method  = method_name,
    tissue  = tissue,
    sex     = sex
  ))
}

