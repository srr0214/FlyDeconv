run_rls <- function(
  ...
  total_mem <- peakRAM::peakRAM({
    ...
    ## -------- 3. 后处理（非 core）--------
    prop_rls <- t(decon$proportions$rls_sig1)
    
    ## 去负值
    prop_rls[prop_rls < 0] <- 0
    
    ## 列归一化
    prop_rls <- sweep(
      prop_rls, 2, colSums(prop_rls), FUN = "/"
    )
    
    prop_rls <- as.data.frame(prop_rls)
  })
  ...
  utils::write.csv(prop_rls, prop_file)
