run_PREDE <- function(
  ...
  if (!requireNamespace("PREDE", quietly = TRUE)) {
    stop("Package 'PREDE' is required.")
  }
  ...
  total_mem <- peakRAM::peakRAM({
    ...
    W1 <- celltype_expr[common_genes, , drop = FALSE]
    ...
    prop_PREDE <- as.data.frame(prop_PREDE)
  })
  ...
  utils::write.csv(prop_PREDE, prop_file)
