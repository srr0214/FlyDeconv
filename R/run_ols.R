run_ols <- function(
  ...
  total_mem <- peakRAM::peakRAM({
    ...
    ## -------- 3. 后处理（非 core）--------
    prop_ols <- t(decon$proportions$ols_sig1)
    
    ## 去负值
    prop_ols[prop_ols < 0] <- 0
    
    ## 列归一化
    prop_ols <- sweep(
      prop_ols, 2, colSums(prop_ols), FUN = "/"
    )
    
    prop_ols <- as.data.frame(prop_ols)
  })
  ...
  utils::write.csv(prop_ols, prop_file)
